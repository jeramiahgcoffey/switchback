 ...
 *
 * Runs automatically at the end of `npm run import-trails` (index.ts calls
 * validateTrails and aborts on failure), and standalone:
 *
...
/**
 * Check every generated trail. Returns a list of "slug: reason" failure
 * strings (empty = all passed). Pure: no process.exit, so callers decide.
 */
export function validateTrails(trails: Trail[]): string[] {
  ...
  for (const t of trails) {
    ...
    // A trail without campsite waypoints can only be planned as a single push
    // (splitIntoDays anchors nights on campsites). Guard the advertised day
    // count against what the planner can actually deliver so the card, the
    // trip-length filter, and the itinerary never disagree.
    const campsites = t.waypoints.filter((w) => w.kind === "campsite").length;
    if (t.estimatedDays > campsites + 1) {
      fail(
        t.slug,
        `estimatedDays ${t.estimatedDays} exceeds campsites+1 (${campsites + 1}); planner cannot split it`,
      );
    }
...
// Standalone CLI: validate the committed generated catalog.
if (process.argv[1]?.endsWith("validate.ts")) {
  const { generatedTrails } = await import("../../lib/data/trails.generated.ts");
  const failures = validateTrails(generatedTrails);
  for (const f of failures) console.error(`FAIL ${f}`);
  console.log(
    failures.length === 0
      ? `OK: ${generatedTrails.length} generated trails passed validation`
      : `${failures.length} failure(s) across ${generatedTrails.length} trails`,
  );
